...
class DataCollatorForLanguageModeling(DataCollator):
    ...
    def collate_batch(self, features: List[Tuple]) -> Dict[str, torch.Tensor]:
        ...
        numbers = [torch.tensor(f['numbers'], dtype=torch.float) for f in features]
        
        input_ids = self._tensorize_batch(input_ids)
        numbers = self._tensorize_number(numbers)
        if self.mlm:
            inputs, labels, numbers = self.mask_tokens(input_ids, numbers)
    ...
    def mask_tokens(self, inputs: torch.Tensor, numbers: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        ...
        numbers[indices_replaced] = 0

        # 10% of the time, we replace masked input tokens with random word
        indices_random = torch.bernoulli(torch.full(labels.shape, 0.5)).bool() & masked_indices & ~indices_replaced
        random_words = torch.randint(len(self.tokenizer), labels.shape, dtype=torch.long)
        inputs[indices_random] = random_words[indices_random]
        numbers[indices_random] = 0

        # The rest of the time (10% of the time) we keep the masked input tokens unchanged
        return inputs, labels, numbers
