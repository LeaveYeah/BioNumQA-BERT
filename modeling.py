...
class NumericEmbedding(nn.Module):
    ...
    def forward(self, embedding, numbers):
        ...
        encoding_table = get_encoding_table(nums, self.hidden_size)
        embedding[indices] = encoding_table
        
        return embedding

class BioNumQABertEmbeddings(nn.Module):
...
BERT_INPUTS_DOCSTRING = r"""
    Args:
        ...
        encoder_attention_mask (:obj:`torch.FloatTensor` of shape :obj:`(batch_size, sequence_length)`, `optional`, defaults to :obj:`None`):
            ...
            ``1`` for tokens that are NOT MASKED, ``0`` for MASKED tokens.
"""    
class BioNumQABertModel(BertPreTrainedModel):
    """
    ...
    def __init__(self, config):
        ...
        self.embeddings = BioNumQABertEmbeddings(config)
...
class BioNumQABertForPreTraining(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

        self.bert = BioNumQABertModel(config)
...
class BioNumQABertForMaskedLM(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
class BioNumQABertForNextSentencePrediction(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
class BioNumQABertForQuestionAnswering(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
